...
public class GameplayUI {
    ...
    private static void drawHeaderEmptyRow(PrintStream out){
        for (int boardCol = 0; boardCol < BOARD_SIZE_IN_SQUARES + 2; ++boardCol) {
            drawHeader(out, " ");
        }
        out.print(SET_BG_COLOR_DARK_GREY);
    ...
    private static void drawEmptyRowSquares(PrintStream out, int color){
        for (int boardCol = 0; boardCol < BOARD_SIZE_IN_SQUARES + 2; ++boardCol) {
            if (boardCol == 0 || boardCol == 9){
                drawSquare(out, " ", SET_BG_COLOR_LIGHT_GREY, SET_TEXT_COLOR_LIGHT_GREY);
            }
            else{
                if(color == 0){
                    drawSquare(out, " ",SET_BG_COLOR_WHITE , SET_TEXT_COLOR_LIGHT_GREY);
                    color = 1;
                }else{
                    drawSquare(out, " ",SET_BG_COLOR_BLACK , SET_TEXT_COLOR_LIGHT_GREY);
                    color = 0;
                }
